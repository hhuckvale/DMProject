...
def amp_cut(row):
    ...
    if V not in amp_windows:
        return False

    low, high = amp_windows[V]
    return (low <= A <= high)

cleaned_df = cleaned_df[cleaned_df.apply(amp_cut, axis=1)]

#15sigma threshold clean
sigma_thresh = pd.to_numeric(cleaned_df["time_above_threshold"], errors="coerce") > 0 #addresses empty cells in csv
...
fit_mask = (bin_centers >=0) & (bin_centers <=5)
...
good_peaks = cleaned_df['peaks_over_threshold'] < 3
cleaned_df = cleaned_df[good_peaks]

#good_peaks = cleaned_df['peaks_over_threshold']
#if cleaned_df['amplitude'] in range (11e-3,19e-3):
 #   good_amp = cleaned_df['amplitude']
  #  cleaned_df = cleaned_df[good_amp]

#low = 11e-3
#high = 19e-3

#mask = (cleaned_df['amplitude'] >= low) & (cleaned_df['amplitude'] <= high)
#cleaned_df = cleaned_df[mask]

#export the cleaned data to another csv
cleaned_df.to_csv(file_path, sep=',', encoding='utf-8-sig', index=True, header=True)

# cuts made = time above threshold, baseline standard deviation